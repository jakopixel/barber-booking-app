...
const safe = u => ({ id: u.id, name: u.name, email: u.email, phone: u.phone, role: u.role, createdAt: u.createdAt, barber: u.barber });
...
r.post('/register', async (req, res) => {
  try {
    ...
    const full = await prisma.user.findUnique({ where: { id: user.id }, include: { barber: true } });
